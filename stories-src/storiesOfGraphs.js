...
const sigmaAction = decorateAction([
  args => [
    {
      type: args[0].type,
      data: { node: args[0].data.node, captor: args[0].data.captor }
    }
  ]
]);
...
storiesOf('Complex graph samples', module)
  ...
  .addWithInfo(
    ...
    () => {
      ...
      let flEasing = select(
        'Easing after layout',
        ['linearNone', 'cubicIn', 'cubicOut', 'cubicInOut'],
        'cubicInOut'
      );
      let flOutboundAttractionDistribution = boolean(
      ...
      return (
        <div style={Note}>
          Play with ForceLink settings under <b>knobs</b>. Check overNode events
          under <b>action logger</b>.
          <Sigma
            renderer="webgl"
            onOverNode={sigmaAction('onOverNode')}
            settings={{ animationsTime: 3000 }}
          >
            <LoadGEXF path={String(process.env.PUBLIC_URL) + '/arctic.gexf'}>
              <ForceLink
                randomize={flRandomize}
                ...
                outboundAttractionDistribution={
                  flOutboundAttractionDistribution
                }
              />
  ...
  .addWithInfo(
    ...
    () => (
      <Sigma renderer="canvas" onClickNode={sigmaAction('onClickNode')}>
        ...
        <LoadJSON
          path={String(process.env.PUBLIC_URL) + '/sites_coords.json'}
        />
      </Sigma>
    )
  )
  .addWithInfo('Small Graph', 'Sample for story', () => {
    return <EdgeLabelSample />;
  })
  .addWithInfo(
    'Big graph animated',
    ...
    () => (
      <Sigma
        ...
        onClickNode={sigmaAction('onClickNode')}
      >
        <LoadJSON
          path={String(process.env.PUBLIC_URL) + '/sites_nocoords.json'}
        >
          <RandomizeNodePositions>
            <ForceAtlas2
              barnesHutOptimize
